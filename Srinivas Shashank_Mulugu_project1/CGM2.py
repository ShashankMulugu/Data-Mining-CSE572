...
cgm_ts2 = pd.read_csv("C:\\Users\\Shashank\\Documents\\IMP\\ASU\\Courses\\CSE 572\\DataFolder\\CGMDatenumLunchPat2.csv")
cgm2 = pd.read_csv("C:\\Users\\Shashank\\Documents\\IMP\\ASU\\Courses\\CSE 572\\DataFolder\\CGMSeriesLunchPat2.csv")


cgm2 = cgm2.dropna(how='all').reset_index(drop=True).interpolate()
cgm_ts2 = cgm_ts2.dropna(how='all').reset_index(drop=True).interpolate()
...
feature = []
fl = []
for i in range(len(cgm2.values)):
    feature_row = []
    # Feature Type 1 - Rolling Window 
    moving_average = moving_avg(cgm2.loc[i,:].tolist(), 5).tolist()
    moving_average = pad(moving_average, 27, np.nan)
    feature_row.extend(moving_average)
    # plt.figure(i)
    # plt.plot(moving_avg(cgm_ts2.loc[i,:].tolist(), 5), moving_avg(cgm2.loc[i,:].tolist(), 5))
    # plt.plot(cgm_ts2.loc[i,:].tolist(), cgm2.loc[i,:].tolist())
    
    
    # Feature Type 2 - Analyzing in the Frequency Domain - FFT & PSD
    y_values = cgm2.loc[i,:].dropna().unique().tolist()
    y_values_ts = cgm_ts2.loc[i,:].dropna().unique().tolist()
    num = len(y_values)
    ...
    feature_row.extend([statistics.stdev(cgm2.loc[i,:])])
    
    feature_row.extend([max(cgm2.loc[i,:])])
    
    feature_row.append(np.sqrt(np.mean(cgm2.loc[i,:]**2)))
...
feature_2D_arr = np.asarray(feature).reshape(37,120)
...
fig.show()
fig.write_image("C:\\Users\\Shashank\\Documents\\IMP\\ASU\\Courses\\CSE 572\\DataFolder\\fig2.png")
