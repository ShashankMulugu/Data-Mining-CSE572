# -*- coding: utf-8 -*-
"""
Created on Wed Feb 12 22:44:29 2020
...
cgm_ts5 = pd.read_csv("C:\\Users\\Shashank\\Documents\\IMP\\ASU\\Courses\\CSE 572\\DataFolder\\CGMDatenumLunchPat5.csv")
cgm5 = pd.read_csv("C:\\Users\\Shashank\\Documents\\IMP\\ASU\\Courses\\CSE 572\\DataFolder\\CGMSeriesLunchPat5.csv")



cgm5 = cgm5.dropna(how='all').reset_index(drop=True).interpolate()
cgm_ts5 = cgm_ts5.dropna(how='all').reset_index(drop=True).interpolate()

# for i in range(0, len(cgm5.index)):
#     for j in range(0, len(cgm5.columns)):
#         print (cgm5.values[i,j],cgm_ts3.values[i,j],i,j)
...
fl = []
il = []
for i in range(len(cgm5.values)):
    feature_row = []
    # Feature Type 1 - Rolling Window 
    moving_average = moving_avg(cgm5.loc[i,:].tolist(), 5).tolist()
    moving_average = pad(moving_average, 38, np.nan)
    feature_row.extend(moving_average)
    # plt.figure(i)
    # plt.plot(moving_avg(cgm_ts5.loc[i,:].tolist(), 5), moving_avg(cgm2.loc[i,:].tolist(), 5))
    # plt.plot(cgm_ts5.loc[i,:].tolist(), cgm2.loc[i,:].tolist())
    
    il.append(len(moving_average))
    
    # Feature Type 2 - Analyzing in the Frequency Domain - FFT & PSD
    y_values = cgm5.loc[i,:].dropna().unique().tolist()
    y_values_ts = cgm_ts5.loc[i,:].dropna().unique().tolist()
    num = len(y_values)
    ...
    feature_row.extend([statistics.stdev(cgm5.loc[i,:])])
    
    feature_row.extend([max(cgm5.loc[i,:])])
    
    feature_row.append(np.sqrt(np.mean(cgm5.loc[i,:]**2)))
...
fig.show()
fig.write_image("C:\\Users\\Shashank\\Documents\\IMP\\ASU\\Courses\\CSE 572\\DataFolder\\fig5.png")
#-----------------------------------------------------------------------