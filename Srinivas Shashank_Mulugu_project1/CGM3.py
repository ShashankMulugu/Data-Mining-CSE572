...
cgm_ts3 = pd.read_csv("C:\\Users\\Shashank\\Documents\\IMP\\ASU\\Courses\\CSE 572\\DataFolder\\CGMDatenumLunchPat3.csv")
cgm3 = pd.read_csv("C:\\Users\\Shashank\\Documents\\IMP\\ASU\\Courses\\CSE 572\\DataFolder\\CGMSeriesLunchPat3.csv")



cgm3 = cgm3.dropna(how='all').reset_index(drop=True).interpolate()
cgm_ts3 = cgm_ts3.dropna(how='all').reset_index(drop=True).interpolate()

# for i in range(0, len(cgm3.index)):
#     for j in range(0, len(cgm3.columns)):
...
feature = []
fl = []
for i in range(len(cgm3.values)):
    feature_row = []
    # Feature Type 1 - Rolling Window 
    moving_average = moving_avg(cgm3.loc[i,:].tolist(), 5).tolist()
    moving_average = pad(moving_average, 27, np.nan)
    feature_row.extend(moving_average)
    # plt.figure(i)
    # plt.plot(moving_avg(cgm_ts3.loc[i,:].tolist(), 5), moving_avg(cgm3.loc[i,:].tolist(), 5))
    # plt.plot(cgm_ts3.loc[i,:].tolist(), cgm2.loc[i,:].tolist())
    
    # Feature Type 2 - Analyzing in the Frequency Domain - FFT & PSD
    y_values = cgm3.loc[i,:].dropna().unique().tolist()
    y_values_ts = cgm_ts3.loc[i,:].dropna().unique().tolist()
    num = len(y_values)
    ...
    feature_row.extend([statistics.stdev(cgm3.loc[i,:])])
    
    feature_row.extend([max(cgm3.loc[i,:])])
    
    feature_row.append(np.sqrt(np.mean(cgm3.loc[i,:]**2)))
    ...
    velocity = pad(velocity, 33, np.nan)
...
feature_2D_arr = np.asarray(feature).reshape(len(feature),120)
...
fig.show()
fig.write_image("C:\\Users\\Shashank\\Documents\\IMP\\ASU\\Courses\\CSE 572\\DataFolder\\fig3.png")
